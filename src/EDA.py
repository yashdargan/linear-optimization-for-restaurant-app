...
def EDA(df):
    ...
    df[feature] = (
    (df["created_at"].dt.minute) + 1 if feature == 'Minute' else
    (df["created_at"].dt.hour) + 1 if feature == 'Hour' else
    (df["created_at"].dt.dayofweek) + 1 if feature == 'Day' else
    (df["created_at"].apply(categorize_shift)) if feature == 'Shift' else 
    None
    )
    # graphs
